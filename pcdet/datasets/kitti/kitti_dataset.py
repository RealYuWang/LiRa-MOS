...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    dataset.set_split(train_split) # 读取ImageSets中的对应划分 txt，将帧号存入sample_id_list
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        #     data_path=ROOT_DIR / 'data' / 'radar_5frames',
        #     save_path=ROOT_DIR / 'data' / 'radar_5frames'
        # )

        # python -m pcdet.datasets.kitti.kitti_dataset create_kitti_infos cfgs/dataset_configs/vod_bimodal.yaml
        dataset = KittiDataset(dataset_cfg, ['Car', 'Pedestrian', 'Cyclist'], True, Path('/home/yu/OpenPCDet/data/my_radar_5frames/'))
        data_dict = dataset[10]
        print(data_dict.keys())
        # print(data_dict['points'].shape)
        # print(data_dict['mos_label'].shape)
        print(data_dict['points_lidar'].shape)
